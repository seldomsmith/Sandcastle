/**
 * Sandcastle vs. Tide Simulator - Terrain Shaders with Stress Heatmap & Topographic Contours
 *
 ...
 * Fragment shader applies tri-planar PBR multi-texture blending, stress heatmap, and topographic isolines.
 */
...
export const terrainFragmentShader = /* glsl */ `
  ...
  void main() {
    ...
    // Topographic Elevation Contour Line Generator (0.05m isolines)
    if (uShowContours) {
      float contourSpacing = 0.05; // 5 cm contour lines
      float val = fract(vElevation / contourSpacing);
      float line = smoothstep(0.0, 0.08, val) - smoothstep(0.92, 1.0, val);
      vec3 contourColor = vec3(0.1, 0.6, 0.95);
      diffuseColor = mix(contourColor, diffuseColor, line);
    }
